...
def run_blend(black_image, white_image, mask):
    ...
    gauss_pyr_mask = assignment4.gaussPyramid(mask, depth)
    gauss_pyr_black = assignment4.gaussPyramid(black_image, depth)
    gauss_pyr_white = assignment4.gaussPyramid(white_image, depth)

    lapl_pyr_black = assignment4.laplPyramid(gauss_pyr_black)
    lapl_pyr_white = assignment4.laplPyramid(gauss_pyr_white)

    outpyr = assignment4.blend(lapl_pyr_white, lapl_pyr_black, gauss_pyr_mask)
    outimg = assignment4.collapse(outpyr)
...
def test_reduce_expand():
    ...
    for red_pyr in reduce1, reduce2:
        for imgin, true_out in zip(red_pyr[0:-1], red_pyr[1:]):
            ...
            usr_out = assignment4.reduce(imgin)
    ...
    expandout = [np.array([[163.2,  102.],
                 ...
                 np.array([[183.6, 114.75, 34.2, 56.25, 101.25, 112.5, 112.5, 112.5, 101.25,  56.25],
                           [204.,  127.5,  38.,  62.5,  112.5,
                               125.,  125., 125.,  112.5,  62.5],
                           [188.1, 119.75, 39.2, 61.25, 106.25,
                            117.5, 117.5, 117.5, 105.75,  58.75],
                           [124.5,  88.75, 44., 56.25,  81.25,
                            87.5,  87.5, 87.5,  78.75,  43.75],
                           [56.4,  52.75, 43.8, 46.25,  51.25,
                            52.5,  52.5, 52.5,  47.25,  26.25],
    ...
    for imgin, true_out in zip(expandin, expandout):
        ...
        usr_out = assignment4.expand(imgin)
...
def test_gaussian_laplacian():
    ...
    for pyr in gauss_pyr1, gauss_pyr2:
        ...
        usr_out = assignment4.gaussPyramid(pyr[0], 3)
    ...
    lapl_pyr1 = [np.array([[-2.95,  -10.04,  -17.67,  -22.09,  -23.02,  -16.73,   -8.97,   -4.01],
                           [-9.82,  -33.47,  -58.9,  -73.63,  -
                               76.75,  -55.78,  -29.9,  -13.39],
                           [-15.57,  -53.07,  161.59,  138.24,
                               133.29,  166.55,  -47.41,  -21.23],
                           [-13.32,  -45.42,  175.06,  155.07,
                               150.83,  179.3,  -40.58,  -18.17],
                           [-8.55,  -29.16,  -51.33,  -64.16,  -
                               66.88,  -48.61,  -26.05,  -11.67],
    ...
    lapl_pyr2 = [np.array([[146.27,  118.15,  101.65,   97.53,  101.65,  118.15,  146.27],
                           [121.16,   93.25,   79.83,   76.48,
                               79.83,   93.25,  121.16],
                           [118.2,   95.65,  -41.91,  -
                               43.79,  -41.91,   95.65,  118.2],
                           [156.61,  142.69,    9.62,    8.85,
                               9.62,  142.69,  156.6],
    ...
    for gauss_pyr, lapl_pyr in zip((gauss_pyr1, gauss_pyr2), (lapl_pyr1, lapl_pyr2)):
        ...
        usr_out = assignment4.laplPyramid(gauss_pyr)
...
def test_blend_collapse():
    ...
    for left_pyr, right_pyr, mask_pyr, out_pyr in ((lapl_pyr11, lapl_pyr12, mask_pyr1, out_pyr1),
        ...
        usr_out = assignment4.blend(left_pyr, right_pyr, mask_pyr)
    ...
    for pyr, img in ((out_pyr1, outimg1), (out_pyr2, outimg2)):
        ...
        usr_out = assignment4.collapse(pyr)
